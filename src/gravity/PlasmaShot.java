...
public class PlasmaShot extends Attack
{
    private Color[] shotParticles;
    public PlasmaShot(Handler handler, float x, float y, double directionInDegrees, boolean gravity, Entity attacker, int attackDMG)
    {
        super(handler, x, y, directionInDegrees, gravity, attacker, attackDMG);
        attackSpeed = 7f;
        shotParticles = Assets.plasmaBlasterColors;
    }
    ...
    protected void createOnHitParticles()
    {
        if (isActive)
        {
            for (int c  = 0; c < 30; c++)
            {
                ...
                handler.getWorld().getParticleManager().addParticle(new Particle(handler, (int)x, (int)y, particleXVel, particleYVel, shotParticles[handler.random(0, 2)], 400));
            }
            isActive = false;
        }
    }
